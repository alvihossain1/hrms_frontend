    ...
    <div>
      <div className='grid grid-cols-12 gap-4 md:p-2'>
        ...
        <div className='col-span-12 md:col-span-6 lg:col-span-3 bg-slate-700 text-slate-200 p-5 rounded-lg flex flex-col justify-between'>
          ...
          <h3 className='my-2'>{employeeData.status === 200 ? employeeActiveCount() : '...'}</h3>
          <p className='my-2'>Total number of Employees Active at the moment.</p>
        </div>
        <div className='col-span-12 md:col-span-6 lg:col-span-3 bg-slate-700 text-slate-200 p-5 rounded-lg flex flex-col justify-between'>
          <h2 className='my-2'>Leave Employees</h2>
          <h3 className='my-2'>{employeeData.status === 200 ? employeeLeaveCount() : '...'}</h3>
          <p className='my-2'>Total number of Employees at Leave at the moment.</p>
        </div>
        <div className='col-span-12 md:col-span-6 lg:col-span-3 bg-slate-700 text-slate-200 p-5 rounded-lg flex flex-col justify-between'>
          <h2 className='my-2'>Today's Attendance</h2>
          <h3 className='my-2'>{attendanceData.status === 200 || attendanceData.status === 0 ? attendanceData.data.length : '..'} / {employeeData.status === 200 ? employeeData.data.length : '..'}</h3>
          <p className='my-2'>Total number of Employees who attended today on {dateFormat(Date.now())}</p>
        </div>
        ...
        {/* --------- ADD DEPARTMENTS ---------- */}
        {session?.user?.module && JSON.parse(session?.user?.module).dept_pos && (
          <>

"use client"
import { updateSalaryAPI, getEmployeeAPI } from '@/lib/api';
...
export default function UpdateSalary() {
  ...
  async function handleSubmit(e, salaryId) {
    e.preventDefault();
    if (baseSalary === 0) {
    ...
    const data = { salaryId, baseSalary, bonus, allowance, benefits, total };
    const response = await updateSalaryAPI(data);
    ...
    return (
      <div className='grid grid-cols-12 mx-0 my-3 md:mx-2 p-2 md:p-4 border-2 border-slate-200 text-slate-700 gap-y-2'>
        <div className='col-span-12'>
          <h5 className='my-1'>Update Salary of <span className='text-purple-500'>{emp.fname} {emp.lname}</span></h5>
        </div>
        <div className='col-span-12'>
        ...
        <div className='col-span-12'>
          <div className='border-b-2 my-2 border-slate-200'>
            <p className='text-sm font-bold my-auto'>Enter New record below</p>
          </div>
        </div>
  ...
  return (
    <div className='flex flex-col gap-x-1.5 gap-y-5 md:gap-x-4 md:gap-y-10 text-slate-800 md:py-12 md:px-12 w-full md:w-10/12 mx-auto'>
      <div>
        <h3 className='text-slate-800'>Update Employee Salary</h3>
      </div>
      <div>
      ...
      <div>
        <h6 className='mb-2 text-slate-700'>Employee's with salaries provided</h6>
